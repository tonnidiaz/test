...
export function getCoinPrecision(
  ...
  switch (plat) {
    case 'binance':
      ...
      pr = Number(
        oType == 'market' ? _i0.quoteAssetPrecision : _i0.baseAssetPrecision
      );
      break;
    case 'bitget':
  ...
  try {
    ...
    switch (plat) {
      case 'binance':
        const _i0 = instru as (typeof binanceInstrus)[0];
        sz = Number(
          _i0.filters.find((el) => el.filterType == 'MARKET_LOT_SIZE')!.minQty
        );
        break;
      case 'bitget':
        const _i1 = instru as (typeof bitgetInstrus)[0];
...
export function getMinAmt(pair: string[], plat: string) {
  ...
  switch (plat) {
    case 'binance':
      const _i0 = instru as (typeof binanceInstrus)[0];
      sz = Number(
        _i0.filters.find((el) => el.filterType == 'NOTIONAL')!.minNotional
      );
      break;
    case 'bitget':
      const _i1 = instru as (typeof bitgetInstrus)[0];
