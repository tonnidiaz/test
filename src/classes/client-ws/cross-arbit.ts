...
export class CrossWs {
    ...
    async addBot(bot: ICrossClientBot, data: CrossArbitData, client: Socket) {
    ...
    async handleTickers({
        ...
        try {
            ...
            client?.emit('/client-ws/book', {type: 'cross', bookA, bookB, pair, platA, platB, perc: _perc, fperc: _fperc})
            return true;
        } catch (e) {
            this._log(e);
            return false;
    ...
    async subUnsub(bot: ICrossClientBot, act: "sub" | "unsub" = "sub") {
        ...
        const unsubA =
            activePairs.findIndex(
                (el) => el.pair == pair.toString() && el.plat == platA
            ) == -1; // pairA not in any of active bots
        const unsubB =
            activePairs.findIndex(
                (el) => el.pair == pair.toString() && el.plat == platB
            ) == -1; // pairA not in any of active bots
        ...
        if (channel1A && channel1B && fn) {
            ...
            if (platA == this.plat) {
                switch (platA) {
                    case "okx":
                        if (act == "sub" || unsubA)
                            fn(channel1A, platA, { instId: symbolA });
                        break;
                    case "bybit":
                    case "kucoin":
                        if (act == "sub" || unsubA)
                            fn(channel1A + symbolA, platA);
                        break;
                }
            }
            if (platB == this.plat) {
                switch (platB) {
                    case "okx":
                        if (act == "sub" || unsubB)
                            fn(channel1B, platB, { instId: symbolB });
                        break;
                    case "bybit":
                    case "kucoin":
                        if (act == "sub" || unsubB)
                            fn(channel1B + symbolB, platB);
                        break;
                }
            }
        }
    ...
    async onMessage(resp: RawData) {
        ...
        if (!r) return this._log("NO parsedData");
        ...
        for (let abot of this.abots) {
            ...
            if (channel == "orderbook" && (this.plat == platA || this.plat == platB)) {
    ...
    _updateBots(abot: ICrossArbitBot) {
