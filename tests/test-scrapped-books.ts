...
import { Bot, Order, TriArbitOrder, TuBook } from "@/models";
...
async function fetchOB({
    ...
    const obA = await TuBook.findOne({ pair: pairA, plat }).exec();
    const obB = await TuBook.findOne({ pair: pairB, plat }).exec();
    const obC = await TuBook.findOne({ pair: pairC, plat }).exec();

    if (!obA || !obB || !obC) return console.log("ONE OF THE BOOKS NOT FOUND");

    let bookA: IObj[] = obA.book; // parseOB(obA.book);
    let bookB: IObj[] = obB.book; // parseOB(obB.book);
    let bookC: IObj[] = obC.book; // parseOB(obC.book);
    console.log({
        bookA: bookA?.length,
        bookB: bookB?.length,
        bookC: bookC?.length,
    ...
    for (let i = 0; i < bookA.length; i++) {
        ...
        const rowB = bookB.find((el) => {
            const tsMs = Date.parse(el.ts);
            return tsMs >= tsMsA && tsMs <= tsMsA + 5000;
        });
        
        const rowC = bookB.find((el) => {
            const tsMs = Date.parse(el.ts);
            return tsMs >= tsMsA && tsMs <= tsMsA + 5000;
        });

        if (!rowB || !rowC) {
            console.log("NO ROW [A/B]")
            continue
        }
...
fetchOB({ plat: "kucoin", A: "USDT", B: "USDC", C: "NEAR" });
