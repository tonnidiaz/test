...
const heroku = false, koyeb = true;
export const BEND_URL = __DEV__
    ? "http://localhost:8000"
    : heroku
    ? "https://tu-trader-3996d65ded90.herokuapp.com"
    : (koyeb ? "https://tu-trader.koyeb.app" : "https://tu-trader-mef0.onrender.com");
export const API_URL = __DEV__ ? "http://localhost:5000" : "https://trader-bend.vercel.app"
...
use_okx = false,
use_gateio = false,
use_mexc = false, use_kucoin = true
   ...
   if (use_bybit)
    ...
    if (use_mexc)
     for (let el of mexcInstrus) {
        if (el.isSpotTradingAllowed && el.status == '1' && el.isSpotTradingAllowed)
            symbols.push([el.baseAsset, el.quoteAsset])
    }
    symbols = Array.from(new Set(symbols.map(el=> el.join('-')))).map(el=> el.split('-'))
...
export const selectParents = (p: string[]) => p.map(e=> ({label: e.toUpperCase(), value: e.toLowerCase()}))
