...
const get_order_book = ()=>{
    return
    const res = axios.get(order_book_URL, {params, headers})
    /* WRITE TO FILE */
    console.log("DONE")
}
...
const readOrderBook = () => { 
 ...
 const readKlines = ()=>{
    ...
    const klines = true ? readJson('data/coin-api/bybit-spot/SOL-USDT.json') : readJson(pth).map(e=> ({ts: e.time_period_start, o: e.price_open, h: e.price_high, l: e.price_low, c: e.price_close, v: e.volume_traded}))
    return klines
 }
const simulateLimitOrder = async (orderPrice, orderType: 'buy' | 'sell') => {
